...
const HireDeveloper1 = () => {
  ...
  const getTeam = async () => {
    try {
      ...
      const res = await axios.get(
        "http://localhost:8080/api/v1/hiring/get-team"
      );
      if (res) {
  ...
  if (!Area) {
    return <p>Error: No Area selected. Please go back and select an area.</p>;
  }
        ...
        <div className="mx-auto px-5 mt-10 grid max-w-7xl grid-cols-2 gap-6 pt-10 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
          {loading ? (
            ...
            <>
              {team.map((expertise) => (
                <Link
                  to={`/hireDeveloper-progress-2?area=${Area}&team=${encodeURIComponent(
                    expertise.title
                  )}`}
                  key={expertise._id}
                >
