    ...
    <>
      <section className="bg-slate-100">
        ...
        <div className="mx-auto max-w-2xl lg:mx-0">
          ...
          <p className="mt-2 text-lg leading-8 text-gray-900 text-center noto">
            Where Expertise Meets Innovation.
          </p>
        </div>
        <hr className="container mt-5" />
        {text && (
          <h6 className="text-center mont text-danger my-3 rob">
            * SELECT AT LEAST ONE AREA OF EXPERTISE *
          </h6>
        )}
        <div className="mx-auto px-5  grid max-w-7xl grid-cols-2 gap-6 pt-10 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
          {loading ? (
            <>
            ...
            <>
              <Bounce triggerOnce={true}>
                {exp.map((expertise) => (
                  <Link
                    to={`/hireDeveloper-progress-1?area=${encodeURIComponent(
                      expertise.title
                    )}`}
                    key={expertise._id}
                  >
